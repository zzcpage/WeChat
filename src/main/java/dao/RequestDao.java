...
public interface RequestDao {
    ...
    //获取发送的请求消息
    @Select("select * from request where suid=#{uid} order by date desc")
    ArrayList<Request> listSendRequest(long uid) ;
    //获取发送和接收到的请求消息
    @Select("select * from request where suid=#{uid} or ruid=#{uid} order by date desc")
...
//    Request getRequest(@Param("id1") long id1 , @Param("id2") long id2) ;
    ...
    //uid2发给uid1的
    @Select("select * from request where (suid=#{uid2} and ruid=#{uid1} and state = 0) order by date desc")
    Request getRequest(@Param("uid1") Long uid1,@Param("uid2") Long uid2);


    @Select("select * from request where (id=#{rid} and state = 0) order by date desc")
    Request getRequestByRid(@Param("rid") Long rid);

    //public List<Request> getRequest(Long uid);
    //获取接收到的请求信息
    @Select("select * from request where ruid=#{uid} order by date desc")
